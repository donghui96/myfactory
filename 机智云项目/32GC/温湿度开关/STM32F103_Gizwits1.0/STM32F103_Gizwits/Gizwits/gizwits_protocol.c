...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagON_OFF_1)
    {
        info->event[info->num] = EVENT_ON_OFF_1;
        info->num++;
        dataPoints->valueON_OFF_1 = gizStandardDecompressionValue(ON_OFF_1_BYTEOFFSET,ON_OFF_1_BITOFFSET,ON_OFF_1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagON_OFF_2)
    {
        info->event[info->num] = EVENT_ON_OFF_2;
        info->num++;
        dataPoints->valueON_OFF_2 = gizStandardDecompressionValue(ON_OFF_2_BYTEOFFSET,ON_OFF_2_BITOFFSET,ON_OFF_2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueON_OFF_1 != cur->valueON_OFF_1)
    {
        GIZWITS_LOG("valueON_OFF_1 Changed\n");
        ret = 1;
    }
    if(last->valueON_OFF_2 != cur->valueON_OFF_2)
    {
        GIZWITS_LOG("valueON_OFF_2 Changed\n");
        ret = 1;
    }

    if(last->valueT_val != cur->valueT_val)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueT_val Changed\n");
            ret = 1;
        }
    }
    if(last->valueH_val != cur->valueH_val)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueH_val Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(ON_OFF_1_BYTEOFFSET,ON_OFF_1_BITOFFSET,ON_OFF_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueON_OFF_1);
    gizStandardCompressValue(ON_OFF_2_BYTEOFFSET,ON_OFF_2_BITOFFSET,ON_OFF_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueON_OFF_2);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueT_val = gizY2X(T_val_RATIO,  T_val_ADDITION, dataPoints->valueT_val); 
    devStatusPtr->valueH_val = gizY2X(H_val_RATIO,  H_val_ADDITION, dataPoints->valueH_val); 
