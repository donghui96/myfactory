...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"a"对应的标识名
	protected static final String KEY_A = "a";
	// 数据点"f"对应的标识名
	protected static final String KEY_F = "f";
	// 数据点"b"对应的标识名
	protected static final String KEY_B = "b";
	// 数据点"c"对应的标识名
	protected static final String KEY_C = "c";
	// 数据点"d"对应的标识名
	protected static final String KEY_D = "d";
	// 数据点"e"对应的标识名
	protected static final String KEY_E = "e";
	 ...
	 */
	// 数据点"a"对应的存储数据
	protected static boolean data_a;
	// 数据点"f"对应的存储数据
	protected static int data_f;
	// 数据点"b"对应的存储数据
	protected static int data_b;
	// 数据点"c"对应的存储数据
	protected static byte[] data_c;
	// 数据点"d"对应的存储数据
	protected static boolean data_d;
	// 数据点"e"对应的存储数据
	protected static boolean data_e;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_A)) {
					data_a = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_F)) {
					data_f = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_B)) {
			
					data_b = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_C)) {
			
					data_c = (byte[]) map.get(dataKey);
				}
				if (dataKey.equals(KEY_D)) {
					data_d = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_E)) {
					data_e = (Boolean) map.get(dataKey);			
				}
			}
		}
