...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flaga)
    {
        info->event[info->num] = EVENT_a;
        info->num++;
        dataPoints->valuea = gizStandardDecompressionValue(a_BYTEOFFSET,a_BITOFFSET,a_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagf)
    {
        info->event[info->num] = EVENT_f;
        info->num++;
        dataPoints->valuef = gizStandardDecompressionValue(f_BYTEOFFSET,f_BITOFFSET,f_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagc)
    {
        info->event[info->num] = EVENT_c;
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valuec,issuedData->attrVals.valuec,sizeof(issuedData->attrVals.valuec));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuea != cur->valuea)
    {
        GIZWITS_LOG("valuea Changed\n");
        ret = 1;
    }
    if(last->valuef != cur->valuef)
    {
        GIZWITS_LOG("valuef Changed\n");
        ret = 1;
    }
    ...
    if(0 != memcmp((uint8_t *)&last->valuec,(uint8_t *)&cur->valuec,sizeof(last->valuec)))
    {
        GIZWITS_LOG("valuec Changed\n");
        ret = 1;
    }
    ...
    if(last->valuee != cur->valuee)
    {
        GIZWITS_LOG("valuee Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(a_BYTEOFFSET,a_BITOFFSET,a_LEN,(uint8_t *)devStatusPtr,dataPoints->valuea);
    if(dataPoints->valuef >= f_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuef Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(f_BYTEOFFSET,f_BITOFFSET,f_LEN,(uint8_t *)devStatusPtr,dataPoints->valuef);  
    }
    gizStandardCompressValue(d_BYTEOFFSET,d_BITOFFSET,d_LEN,(uint8_t *)devStatusPtr,dataPoints->valued);
    gizStandardCompressValue(e_BYTEOFFSET,e_BITOFFSET,e_LEN,(uint8_t *)devStatusPtr,dataPoints->valuee);
    ...
    gizMemcpy((uint8_t *)devStatusPtr->valuec,(uint8_t *)&dataPoints->valuec,sizeof(dataPoints->valuec));
    return 0;
}
