...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagup)
    {
        info->event[info->num] = EVENT_up;
        info->num++;
        dataPoints->valueup = gizX2Y(up_RATIO,  up_ADDITION, issuedData->attrVals.valueup); 
    }
        
    if(0x01 == issuedData->attrFlags.flagdown)
    {
        info->event[info->num] = EVENT_down;
        info->num++;
        dataPoints->valuedown = gizX2Y(down_RATIO,  down_ADDITION, issuedData->attrVals.valuedown); 
    }
        
    if(0x01 == issuedData->attrFlags.flagtime1)
    {
        info->event[info->num] = EVENT_time1;
        info->num++;
        dataPoints->valuetime1 = gizX2Y(time1_RATIO,  time1_ADDITION, issuedData->attrVals.valuetime1); 
    }
        
    if(0x01 == issuedData->attrFlags.flagtime2)
    {
        info->event[info->num] = EVENT_time2;
        info->num++;
        dataPoints->valuetime2 = gizX2Y(time2_RATIO,  time2_ADDITION, issuedData->attrVals.valuetime2); 
    }
        
    if(0x01 == issuedData->attrFlags.flagS)
    {
        info->event[info->num] = EVENT_S;
        info->num++;
        dataPoints->valueS = gizX2Y(S_RATIO,  S_ADDITION, issuedData->attrVals.valueS); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueup != cur->valueup)
    {
        GIZWITS_LOG("valueup Changed\n");
        ret = 1;
    }
    if(last->valuedown != cur->valuedown)
    {
        GIZWITS_LOG("valuedown Changed\n");
        ret = 1;
    }
    if(last->valuetime1 != cur->valuetime1)
    {
        GIZWITS_LOG("valuetime1 Changed\n");
        ret = 1;
    }
    if(last->valuetime2 != cur->valuetime2)
    {
        GIZWITS_LOG("valuetime2 Changed\n");
        ret = 1;
    }
    if(last->valueS != cur->valueS)
    {
        GIZWITS_LOG("valueS Changed\n");
        ret = 1;
    }
    if(last->values != cur->values)
    {
    ...
    if(last->valuet1 != cur->valuet1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet1 Changed\n");
            ret = 1;
        }
    }
    if(last->valuet2 != cur->valuet2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet2 Changed\n");
            ret = 1;
        }
    }
    if(last->valuet3 != cur->valuet3)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet3 Changed\n");
            ret = 1;
        }
    }
    if(last->valuet4 != cur->valuet4)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet4 Changed\n");
            ret = 1;
        }
    }
    if(last->valuet5 != cur->valuet5)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet5 Changed\n");
            ret = 1;
        }
    }
    if(last->valuet6 != cur->valuet6)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet6 Changed\n");
            ret = 1;
        }
    }
    if(last->valueh6 != cur->valueh6)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueh6 Changed\n");
            ret = 1;
        }
    }
    if(last->valuet7 != cur->valuet7)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuet7 Changed\n");
            ret = 1;
        }
    }
    if(last->valueh7 != cur->valueh7)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueh7 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueup = gizY2X(up_RATIO,  up_ADDITION, dataPoints->valueup); 
    devStatusPtr->valuedown = gizY2X(down_RATIO,  down_ADDITION, dataPoints->valuedown); 
    devStatusPtr->valuetime1 = gizY2X(time1_RATIO,  time1_ADDITION, dataPoints->valuetime1); 
    devStatusPtr->valuetime2 = gizY2X(time2_RATIO,  time2_ADDITION, dataPoints->valuetime2); 
    devStatusPtr->valueS = gizY2X(S_RATIO,  S_ADDITION, dataPoints->valueS); 
    devStatusPtr->values = gizY2X(s_RATIO,  s_ADDITION, dataPoints->values); 
    devStatusPtr->valuet1 = gizY2X(t1_RATIO,  t1_ADDITION, dataPoints->valuet1); 
    devStatusPtr->valuet2 = gizY2X(t2_RATIO,  t2_ADDITION, dataPoints->valuet2); 
    devStatusPtr->valuet3 = gizY2X(t3_RATIO,  t3_ADDITION, dataPoints->valuet3); 
    devStatusPtr->valuet4 = gizY2X(t4_RATIO,  t4_ADDITION, dataPoints->valuet4); 
    devStatusPtr->valuet5 = gizY2X(t5_RATIO,  t5_ADDITION, dataPoints->valuet5); 
    devStatusPtr->valuet6 = gizY2X(t6_RATIO,  t6_ADDITION, dataPoints->valuet6); 
    devStatusPtr->valueh6 = gizY2X(h6_RATIO,  h6_ADDITION, dataPoints->valueh6); 
    devStatusPtr->valuet7 = gizY2X(t7_RATIO,  t7_ADDITION, dataPoints->valuet7); 
    devStatusPtr->valueh7 = gizY2X(h7_RATIO,  h7_ADDITION, dataPoints->valueh7); 
    devStatusPtr->valuetime = gizY2X(time_RATIO,  time_ADDITION, dataPoints->valuetime); 
