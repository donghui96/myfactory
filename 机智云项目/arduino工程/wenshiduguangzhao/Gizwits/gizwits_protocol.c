...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagb)
    {
        ...
        info->num++;
        dataPoints->valueb = gizStandardDecompressionValue(b_BYTEOFFSET,b_BITOFFSET,b_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueguangzhao != cur->valueguangzhao)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueguangzhao Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(b_BYTEOFFSET,b_BITOFFSET,b_LEN,(uint8_t *)devStatusPtr,dataPoints->valueb);
    ...
    devStatusPtr->valueguangzhao = exchangeWord(gizY2X(guangzhao_RATIO,  guangzhao_ADDITION, dataPoints->valueguangzhao)); 
