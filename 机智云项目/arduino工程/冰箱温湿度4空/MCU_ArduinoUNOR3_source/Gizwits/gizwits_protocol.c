...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagT)
    {
        info->event[info->num] = EVENT_T;
        info->num++;
        dataPoints->valueT = gizX2Y(T_RATIO,  T_ADDITION, issuedData->attrVals.valueT); 
    }
    ...
    if(0x01 == issuedData->attrFlags.flagH)
    {
        info->event[info->num] = EVENT_H;
        info->num++;
        dataPoints->valueH = gizX2Y(H_RATIO,  H_ADDITION, issuedData->attrVals.valueH); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueT != cur->valueT)
    {
        GIZWITS_LOG("valueT Changed\n");
        ret = 1;
    }
    if(last->valuet != cur->valuet)
    {
    ...
    if(last->valueH != cur->valueH)
    {
        GIZWITS_LOG("valueH Changed\n");
        ret = 1;
    }
    if(last->valueh != cur->valueh)
    {
    ...
    if(last->valueh1 != cur->valueh1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueh1 Changed\n");
            ret = 1;
        }
    }
    if(last->valueD != cur->valueD)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueD Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueT = gizY2X(T_RATIO,  T_ADDITION, dataPoints->valueT); 
    devStatusPtr->valuet = gizY2X(t_RATIO,  t_ADDITION, dataPoints->valuet); 
    devStatusPtr->valueH = gizY2X(H_RATIO,  H_ADDITION, dataPoints->valueH); 
    devStatusPtr->valueh = gizY2X(h_RATIO,  h_ADDITION, dataPoints->valueh); 
    ...
    devStatusPtr->valueh1 = gizY2X(h1_RATIO,  h1_ADDITION, dataPoints->valueh1); 
    devStatusPtr->valueD = gizY2X(D_RATIO,  D_ADDITION, dataPoints->valueD); 
