...
void loop() {  
  float a = dht11_0.readTemperature();//读湿度
    float e = dht11_0.readHumidity(); //读湿度
     int val=analogRead(A1);
  KEY_Handle();//key handle , network configure
  wifiStatusHandle();//WIFI Status Handle
  
  long varW_t = a;//Add Sensor Data Collection
  myGizwits.write(VALUE_t, varW_t);
  long varW_h = e;//Add Sensor Data Collection
  myGizwits.write(VALUE_h, varW_h);
  unsigned long varW_y = val;//Add Sensor Data Collection
