...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagdeng)
    {
        info->event[info->num] = EVENT_deng;
        info->num++;
        dataPoints->valuedeng = gizStandardDecompressionValue(deng_BYTEOFFSET,deng_BITOFFSET,deng_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuedeng != cur->valuedeng)
    {
        GIZWITS_LOG("valuedeng Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(deng_BYTEOFFSET,deng_BITOFFSET,deng_LEN,(uint8_t *)devStatusPtr,dataPoints->valuedeng);
